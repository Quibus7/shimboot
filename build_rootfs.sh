...
#build the debian/ubuntu/alpine/arch rootfs
...
print_help() {
  ...
  echo "  distro          - The Linux distro to use. This should be 'debian', 'ubuntu', 'alpine', or 'arch'."
...
if [ "$distro" = "debian" ]; then
  ...
  print_info "downloading arch linux bootstrap"
  real_arch="x86_64"
  if [ "$arch" = "arm64" ]; then
    real_arch="aarch64"
  fi

  mirror_url="https://mirrors.kernel.org/archlinux/iso/latest"
  tarball_name="archlinux-bootstrap-$real_arch.tar.zst"
  bootstrap_tarball="/tmp/$tarball_name"
  bootstrap_dir="/tmp/arch-bootstrap"

  wget -q --show-progress "$mirror_url/$tarball_name" -O "$bootstrap_tarball"

  print_info "extracting arch linux bootstrap"
  rm -rf "$bootstrap_dir"
  mkdir -p "$bootstrap_dir"
  tar -xf "$bootstrap_tarball" -C "$bootstrap_dir" --numeric-owner
  rm -f "$bootstrap_tarball"

  arch_bootstrap_root="$bootstrap_dir/root.$real_arch"
  echo "Server = https://mirrors.kernel.org/archlinux/\$repo/os/\$arch" > "$arch_bootstrap_root/etc/pacman.d/mirrorlist"

  print_info "bootstrapping arch linux rootfs with pacstrap"
  arch_bootstrap_mounts="proc sys dev run"
  for mountpoint in $arch_bootstrap_mounts; do
    mount --make-rslave --rbind "/$mountpoint" "$arch_bootstrap_root/$mountpoint"
  done
  mount --bind "$rootfs_dir" "$arch_bootstrap_root/mnt"

  chroot "$arch_bootstrap_root" pacman-key --init
  chroot "$arch_bootstrap_root" pacman-key --populate archlinux
  chroot "$arch_bootstrap_root" pacstrap /mnt base

  umount "$arch_bootstrap_root/mnt"
  for mountpoint in $arch_bootstrap_mounts; do
    umount -l "$arch_bootstrap_root/$mountpoint"
  done
  rm -rf "$bootstrap_dir"
